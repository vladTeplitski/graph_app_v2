...
function pullData(url, chartType){
  ...
  .then(function(data) {
    ...
    deploy_data_for_construction(data, chartType);
  })
...
function deploy_data_for_construction(data, chartType){
  //Navigate to the data construction
  _debugLogger(data.History.Entry[0]);
  _debugLogger('[!] Constructing data for: ' + chartType);
}
