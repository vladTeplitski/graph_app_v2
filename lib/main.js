...
function _constructAPIURL(from_date, to_date){
  ...
  _api_url = _api_domain + _api_endpoint + _constructAPIQUERY(from_date, to_date);
  return _api_url;
}

function _constructAPIQUERY(from_date, to_date){
  _debugLogger("[i] into _constructAPIQUERY()");
  _api_query = "?stock_number[0]=" + _api_stock_number  + "&api_key=" + _api_key + "&from_date=" + from_date + "&to_date=" + to_date;
  return _api_query;
}
...
function build_weekly(){
  ...
  //pull data
  raw_data = pullData(_constructAPIURL(_api_date_from, _api_date_to));
  _debugLogger(raw_data.History.Entry[0]);
